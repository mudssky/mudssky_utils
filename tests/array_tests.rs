...
fn test_chunk() {
    ...
    // Test edge cases
    assert_eq!(chunk(&data, 1), vec![vec![1], vec![2], vec![3], vec![4], vec![5], vec![6], vec![7]]);
...
fn test_unique() {
    ...
    assert_eq!(unique_names, vec![("Alice", 25), ("Bob", 30), ("Alice", 35)]);
    ...
    assert_eq!(unique(&already_unique, Some(|&x: &i32| x)), vec![1, 2, 3, 4]);
